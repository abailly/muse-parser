 ...
 * 
 */
public class ImageMapper extends PathMapper {

  private static final Pattern pat = Pattern
      .compile("(.*)\\.(png|gif|jpeg|tiff)");

  public ImageMapper(PathMapper next) {
    super(next);
  }
  ...
  protected Page doMap(String path) {
    ...
    if (m.matches()) {
      ...
      return new ImagePage("image/" + m.group(2), new File(getRoot(), path));
    }
    return null;
  }
}
