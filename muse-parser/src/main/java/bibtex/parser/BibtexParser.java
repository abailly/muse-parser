...
/**
 * The parser will parse the bibtex into a basic AST. Have a look at the different
 * Expanders defined in the bibtex.expansions package if you need more than that.
 * 
 * @author henkel
 */
public final class BibtexParser {
	...
	/**
	 * @param throwAllParseExceptions Setting this to true means that all exceptions will be
	 * thrown immediately. Otherwise, the parser will skip over things it can't parse and
	 * you can use getExceptions to retrieve the exceptions later.
	 */
	 ...
	 * afterwards (if you don't use throwAllParseExceptions which you can configure
	 * in the constructor)...
	 * 
	 * @param bibtexFile
	 * @param input
	...
	private void parseEntry() throws ParseException, IOException {
		...
		final int bracketChoice =
			lexer.scanAlternatives(new char[] { '{', '(' }, false);
			...
			while (true) {
				...
				if (lexer.currentInputChar() == ',') {
					...
					if (lexer.currentInputChar() == '}')
						break;
					String fieldName =
						lexer.scanLiteral(EXCEPTION_SET_NAMES, true, true);
					lexer.scan('=');
					BibtexAbstractValue value = parseValue();
	...
	private BibtexAbstractValue parseValue() throws ParseException, IOException {
		...
		if (inputCharacter == '"') {
			...
			String stringContent =
				lexer.scanLiteral(EXCEPTION_SET_NAMES, false, true).trim();
